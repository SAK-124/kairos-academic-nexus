...
type ContentSections = Record<string, any>;
...
const ensureSection = (sections: ContentSections, key: string, fallback: any) => {
  if (!sections[key]) {
...
export const AdminPanel = ({ onClose }: AdminPanelProps) => {
  ...
  const loadContentSections = async () => {
    ...
    data?.forEach((section) => {
      next[section.section_name] = section.content ?? {};
    });

    ensureSection(next, "hero", {
    ...
    ensureSection(next, "social-proof", {
    ...
    ensureSection(next, "pricing", {
      plans: [],
    });

    ensureSection(next, "faq", {
      items: [],
    });

    ensureSection(next, "ai", {
  ...
  const socialContent = contentSections["social-proof"] ?? {};
  const faqContent = contentSections["faq"] ?? { items: [] };
  const aiContent = contentSections["ai"] ?? { defaultPrompt: "You are Kairos, an academic co-pilot." };
  const globalAnimations = (animationSettings["global"] as any) ?? {};
  const introAnimations = (animationSettings["intro"] as any) ?? {};
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setContentSections((prev) => ({
                              ...
                              faq: {
                                ...faqContent,
                                items: (faqContent.items ?? []).filter((_: any, idx: number) => idx !== index),
                              },
                            }))
                          }
                      ...
                      <Input
                        ...
                        onChange={(event) =>
                          setContentSections((prev) => ({
                            ...
                            faq: {
                              ...faqContent,
                              items: (faqContent.items ?? []).map((faqItem: any, idx: number) =>
