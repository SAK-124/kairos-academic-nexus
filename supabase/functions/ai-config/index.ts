...
const isAdminUser = (user: SupabaseUser | null) => {
  if (!user) return false;
  const appRoles = Array.isArray((user.app_metadata as { roles?: string[] } | null)?.roles)
    ? ((user.app_metadata as { roles?: string[] }).roles as string[])
    : [];
  return (
    ...
    (user.user_metadata as Record<string, unknown>)?.is_admin === true ||
